function OpenOption(id) {
  if (localStorage.getItem("Sesion") === "true") {
    ...
    $("#root").load(id + ".html", function (responseTxt, statusTxt, xhr) {
      if (statusTxt == "success") {
        //Menu class control
        // $(".blue-menu").addClass("w3-text-grey").removeClass("blue-menu");
        $("#" + id)
          .removeClass("w3-text-grey")
          .addClass("blue-menu");
...
function OpenOtherOption(id, titulo) {
  {
    $.ajax({
      url: id + ".html",
      type: "post",
      ...
      success: function (response) {
        if (response == "") {
          NotifiAlert(
            "Hay un problema con la conexión al servidor",
            "w3-red",
            6000
          );
        } else {
          $("#root").html(response);
          $("#titname").html(titulo);
          history.pushState(titulo, titulo, titulo);
          window.scrollTo(0, 0);
        }
      },
